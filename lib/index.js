...
import { importLocalMcp, localMcpInventory } from './mcp-import.js'
...
async function handleMcpInventory(req, res, loader) {
  if (req.method !== 'GET') return json(res, 405, { ok: false, error: 'method-not-allowed' })
  if (!isSameOriginBrowserRequest(req)) return json(res, 403, { ok: false, error: 'forbidden' })
  try {
    return json(res, 200, { ok: true, servers: await localMcpInventory(loader) })
  } catch (error) {
    return json(res, 500, { ok: false, error: error instanceof Error ? error.message : String(error) })
  }
}

async function handleMcpImport(req, res, loader) {
  if (req.method !== 'POST') return json(res, 405, { ok: false, error: 'method-not-allowed' })
  if (!isSameOriginBrowserRequest(req)) return json(res, 403, { ok: false, error: 'forbidden' })
  ...
  try {
    const payload = JSON.parse(await readBody(req))
    if (typeof payload.id !== 'string' || !/^[a-f0-9]{12}$/u.test(payload.id)) throw new Error('invalid MCP server id')
    return json(res, 200, { ok: true, server: await importLocalMcp(loader, payload.id) })
  } catch (error) {
...
export function apply(ctx) {
  ...
  const disposeMcpInventory = ctx.webServer.register({
    kind: 'exact',
    path: MCP_INVENTORY_ROUTE,
    handler: (req, res) => handleMcpInventory(req, res, ctx.loader),
  })
  const disposeMcpImport = ctx.webServer.register({
    kind: 'exact',
    path: MCP_IMPORT_ROUTE,
    handler: (req, res) => handleMcpImport(req, res, ctx.loader),
  })
  return () => {
    disposeMcpImport()
    disposeMcpInventory()
...
export { CONTEXT_ROUTE, MCP_IMPORT_ROUTE, MCP_INVENTORY_ROUTE, ROUTE }
